...
export default function StorageManagerModal({ isOpen, onClose, onStorageCleared }: StorageManagerModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-pine-950/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-150">
      <div className="bg-white w-full max-w-2xl rounded-2xl border border-sand-200 shadow-2xl overflow-hidden my-8 animate-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="p-5 sm:p-6 border-b border-sand-200 flex items-center justify-between bg-sand-50/50">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-pine-50 border border-pine-100 rounded-xl text-pine-700">
              <HardDrive className="h-6 w-6" />
            </div>
            <div>
              <h3 className="text-lg sm:text-xl font-extrabold text-ink-900 tracking-tight flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-lg text-ink-400 hover:text-ink-700 hover:bg-sand-100 transition-colors cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-5 sm:p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          {/* Usage Meter Card */}
          <div className="p-4 bg-sand-50/80 border border-sand-200 rounded-xl space-y-3">
            <div className="flex items-center justify-between text-xs">
              <span className="font-bold text-ink-700 flex items-center gap-1.5">
                <Database className="h-3.5 w-3.5 text-pine-700" />
                <span>Total Storage Used</span>
              </span>
              <span className="font-extrabold text-ink-900 font-mono text-sm">
                {formatSize(totalBytes)} <span className="text-ink-400 font-normal text-xs">/ ~5.0 MB ({usedPercentage.toFixed(1)}%)</span>
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-[11px] text-ink-500 leading-relaxed flex-wrap gap-1">
              <span>Standard web browsers allocate <strong>5 MB to 10 MB</strong> for local storage.</span>
              <Link 
                href="/faq" 
                onClick={onClose}
                className="text-terracotta-600 hover:text-terracotta-700 font-semibold underline whitespace-nowrap"
              >
                Privacy & AI FAQ →
              </Link>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[11px] font-semibold text-ink-500 hover:text-terracotta-600 flex items-center gap-1 cursor-pointer transition-colors"
              >
            ...
            <div className="space-y-2">
              {categories.map((cat) => {
                ...
                return (
                  <div
                    key={cat.id}
                    className="p-3.5 bg-white border border-sand-200 hover:border-sand-300 rounded-xl transition-all flex items-center justify-between gap-3 shadow-2xs"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`p-2 rounded-lg flex-shrink-0 ${hasData ? 'bg-terracotta-50 text-terracotta-600 border border-terracotta-100' : 'bg-sand-100 text-ink-400'}`}>
                        <IconComponent className="h-4 w-4" />
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-ink-900 truncate">{cat.name}</span>
                          {cat.itemCount > 0 && (
                            <span className="text-[10px] font-semibold text-ink-600 bg-sand-100 px-1.5 py-0.2 rounded">
                              {cat.itemCount} {cat.itemCount === 1 ? 'item' : 'items'}
                            </span>
                          )}
                        </div>
                        <p className="text-[11px] text-ink-500 truncate">{cat.description}</p>
                      </div>
                    </div>

                    <div className="flex items-center gap-3 flex-shrink-0">
                      <span className="text-xs font-mono font-bold text-ink-700">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg text-ink-400 hover:text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-100 transition-colors disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-ink-400 cursor-pointer disabled:cursor-not-allowed"
                      >
          ...
          <div className="border border-sand-200 rounded-xl overflow-hidden bg-sand-50/50">
            <button
              type="button"
              onClick={() => setShowManualGuide(!showManualGuide)}
              className="w-full p-3.5 flex items-center justify-between text-left text-xs font-bold text-ink-800 hover:bg-sand-100/70 transition-colors cursor-pointer"
            >
              <div className="flex items-center gap-2">
                <Info className="h-4 w-4 text-pine-700" />
                <span>How to clear storage manually in your browser</span>
              </div>
              {showManualGuide ? <ChevronUp className="h-4 w-4 text-ink-400" /> : <ChevronDown className="h-4 w-4 text-ink-400" />}
            </button>

            {showManualGuide && (
              <div className="p-4 border-t border-sand-200 bg-white text-xs text-ink-600 space-y-3 leading-relaxed">
                <div>
                  <strong className="text-ink-900">Google Chrome / Microsoft Edge:</strong>
                  <p className="mt-0.5">
                    1. Press <kbd className="px-1.5 py-0.5 bg-sand-100 border border-sand-200 rounded font-mono text-[10px]">F12</kbd> (or Right-Click → <em>Inspect</em>).<br/>
                    ...
                    4. Click on your Ascent website URL, then right-click → <strong>Clear</strong>, or click the clear icon (<Trash2 className="h-3 w-3 inline text-ink-500" />).
                  </p>
                </div>

                <div>
                  <strong className="text-ink-900">Apple Safari (Mac):</strong>
                  <p className="mt-0.5">
                ...
                <div>
                  <strong className="text-ink-900">Mozilla Firefox:</strong>
                  <p className="mt-0.5">
                    1. Press <kbd className="px-1.5 py-0.5 bg-sand-100 border border-sand-200 rounded font-mono text-[10px]">F12</kbd> → select the <strong>Storage</strong> tab.<br/>
        ...
        {/* Footer Actions */}
        <div className="p-4 sm:p-5 border-t border-sand-200 bg-sand-50 flex items-center justify-between flex-wrap gap-3">
          <button
            type="button"
          ...
          <button
            type="button"
            onClick={onClose}
            className="text-xs font-bold text-ink-700 hover:text-ink-900 bg-white hover:bg-sand-100 border border-sand-200 px-4 py-2 rounded-xl transition-colors cursor-pointer shadow-2xs"
          >
            Close
          </button>
        </div>
